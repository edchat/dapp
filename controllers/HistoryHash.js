	...
	function (dcl, lang, topic, on, Controller, hash) {
		...
		return dcl(Controller, {
			...
			constructor: function (app) {
				...
				if (this.app.domNode) {
					this.onDomNodeChange({
						oldNode: null,
						newNode: this.app.domNode
					});
				}
			...
			_onHashChange: function (currentHash) {
				...
				if (this._oldHistoryLen > this._newHistoryLen) {
					...
					this._historyStack.splice(
						(this._newHistoryLen - this._historyDiff - 1), (this._historyStack.length - 1));
				...
				if (this._historyLen < window.history.length) {
					...
					if (!this._startTransitionEvent) {
						...
						this.app.emit("app-transition", {
							...
							opts: {
								params: hash.getParams(currentHash) || {}
							}
						});
					}
				} else {
			...
			_back: function (currentHash, detail) {
				...
				topic.publish("/app/history/back", {
					"viewId": target,
					"detail": detail
				});
			...
			_forward: function (currentHash, detail) {
				...
				topic.publish("/app/history/forward", {
					"viewId": target,
					"detail": detail
				});
