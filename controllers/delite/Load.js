define(
	...
	function (require, dcl, on, lang, Deferred, Controller, constraints) {
		...
		var resolveView = function (event, name, newView, parentView) {
			// in addition to arguments required by delite we pass our own needed arguments
			// to get them back in the transitionDeferred
			console.log("Load resolveView called with name =["+name+"], nextView.id = ["+ newView.id+"], parentView.id = ["+parentView.id+"]");
		...
		return dcl(Controller, {
			...
			_loadHandler: function (event) {
				...
				console.log("dapp/delite/controller/Load:_loadHandler called for event.dest=" + event.dest);
				var self = this;
				...
				if (!event.parent) {
					console.log("Load _loadHandler called without event.parent!!!!!!! for "+event.dest)
					...
					console.log("Load _loadHandler called without event.parent set it to = "+event.parent.id);
				} else {
					console.log("Load _loadHandler called with event.parent set it to = "+event.parent.id);
				}
				var view = null;
				...
				event.loadDeferred.then(function (value) {
					...
					if (value.dapp.name) { // need to call the before Activate/Deactivate for the view(s)
						toId = value.dapp.name;
						subIds = null;
						var viewTarget = self.app.flatViewDefinitions[value.dapp.name].viewPath;
						parent = value.parent;
						if (viewTarget) {
					...
					//	}
						...
						if (!value.isParent) {
							...
							console.log(F + " calling _handleBeforeDeactivateCalls next id=[", next.id,
								"], parent.id=["+ next.parent.id+ "] currentSubViewArray =",currentSubViewArray);
			...
			_createView: function (event, name, params, parent, type) {
				...
				require([type ? type : "../../View"], function (View) {
					...
					new View(params).start().then(function (newView) {
						...
						resolveView(event, name, newView, pTest);
					});
				});
			},
			...
			_handleBeforeActivateCalls: function (subs, current, data /*, subIds*/ ) {
				...
				for (var i = subs.length - 1; i >= 0; i--) {
					...
					if(this.app.getViewDefFromViewName(v.parent.id)){
			...
			_getCurrentSubViewArray: function (parent, nextSubViewArray, removeView) {
				...
				for (var i = nextSubViewArray.length - 1; i >= 0; i--) {
					...
					constraint = nextSubViewArray[i].parentSelector || nextSubViewArray[i].parent.id;
